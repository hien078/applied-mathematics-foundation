# Numerical Computing Foundations

## Contents

| File | Type | Description |
|---|---|---|
| `floating_point_stability.md` | Theory | IEEE 754, error analysis |
| `conditioning_stability.ipynb` | Computation | Condition number demos |
| `vectorization_numpy.ipynb` | Computation | Performance benchmarks |

## Used By

All topics benefit from numerical stability awareness. Critical for:
- [13 Neural Networks](../../topics/13_neural_networks/README.md) — gradient stability
- [02 Gradient Descent](../../topics/02_gradient_descent/README.md) — convergence issues
